...
function Categoreis(){
    ...
    async function fetchData(){
        ...
        try{
            const q = query(collection(db, 'places'), where('type', '==', category), limit(4));
    ...
    async function fetchMoreData(){
        const category = params.categoryName;
        try{
            const q = query(collection(db, 'places'), where('type', '==', category), limit(2), startAfter(lastFetched));
            const dataSnap = await getDocs(q);
            if(dataSnap){
                const lastVisible = dataSnap.docs[dataSnap.docs.length - 1];
                setLastFetched(lastVisible);
    ...
    return(
        <SectionWrraper>
            <SectionHeader title={`places for ${params.categoryName}`}/>
